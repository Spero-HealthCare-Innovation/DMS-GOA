...
rain_severity_df = severity_df.filter(col("hazard_types") == "rainfall") \
    .select("hazard_rng_low", "hazard_rng_medium", "hazard_rng_high", "unit") \
    .limit(1)
rain_severity = rain_severity_df.collect()[0].asDict()
...
parsed_df = kafka_df.selectExpr("CAST(value AS STRING) as json_str") \
    ...
    .withColumn(
        "alert_type",
        when(col("rain") >= lit(rain_severity["hazard_rng_high"]), lit(1))
        .when(col("rain") >= lit(rain_severity["hazard_rng_medium"]), lit(2))
        .when(col("rain") >= lit(rain_severity["hazard_rng_low"]), lit(3))
        .otherwise(lit(4))
)

spark.catalog.refreshTable("raw_dms.raw_weather_alerts")
hive_table_name = "raw_dms.raw_weather_alerts"
spark.table("raw_dms.raw_weather_alerts").printSchema()
