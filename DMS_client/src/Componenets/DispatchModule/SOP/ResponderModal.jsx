...
export default function ResponderModal({
  ...
  // 🚀 yaha hum assigned vehicles ko persist rakhenge
  ...
  // const fetchVehicles = async (baseId = "", responderId = "") => {
  //   setLoading(true);
  //   try {
  //     let url = `${port}/DMS_mdt/vehical/`;
  //     if (baseId && responderId) {
  //       url += `?veh_base_loc=${baseId}&responder=${responderId}`;
  //     } else if (baseId) {
  //       url += `?veh_base_loc=${baseId}`;
  //     } else if (responderId) {
  //       url += `?responder=${responderId}`;
  //     }
  ...
  //       assigned: assignedVehicles[v.veh_id] || false, // ✅ merge old state
  //     }));
    ...
    try {
      const res = await axios.get(`${port}/DMS_mdt/vehical/`, {
        params: {
          ...
          veh_num: vehicleNo || undefined, // ✅ vehicle filter
          lat: lattitude,
          long: longitude,
        },
      });
  ...
  // jab modal open ho to base locations aur vehicles fetch karo
  useEffect(() => {
    if (open) {
      fetchBaseLocations();
      setSelectedResponder("all"); // ✅ default All Vehicles tab select
      fetchVehicles("", ""); // ✅ all vehicles api call
    }
  }, [open]);

  // jab filters change ho to nayi vehicles fetch karo
  useEffect(() => {
    if (selectedResponder === "all") {
      fetchVehicles(selectedBaseLocation, ""); // ✅ all vehicles
    } else if (selectedResponder) {
      fetchVehicles(selectedBaseLocation, selectedResponder); // ✅ specific responder
    }
  }, [selectedResponder, selectedBaseLocation, lattitude, longitude]);

  // Default responder select if not passed
  useEffect(() => {
  ...
  const resetFilters = () => {
    ...
    setVehicleNo("");
    setSelectedResponder("all"); // ✅ reset responder to All Vehicles
    fetchVehicles("", "", ""); // ✅ fetch all vehicles
  };
  const handleSave = () => {
    onSave({
      ...
      vehicleNo,
      assignedVehicles, // ✅ parent ko bhejenge
    });
  };
...
>
  ...
  <Select
    ...
    sx={{
      ...
      "&:hover:not(.Mui-disabled):before": { borderBottom: "1px solid #1976d2" },
      "& .MuiSelect-select": {
        padding: "8px 12px",
        ...
        {!loading && (
          <TableContainer
            ...
            sx={{
              ...
              "&::-webkit-scrollbar-thumb:hover": { backgroundColor: "#2593ecff" },
